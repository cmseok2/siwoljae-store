...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  if (request.method !== "POST") return response.status(405).json({ message: "지원하지 않는 요청입니다." });
  if (origin && origin !== allowedOrigin) return response.status(403).json({ message: "허용되지 않은 요청입니다." });

  try {
    const input = request.body as Input;
    if (!input.privacy || !input.birthDate || !input.gender || !input.calendarType || (!input.unknownTime && !input.birthTime)) {
      return response.status(400).json({ message: "생년월일시와 성별을 모두 확인해 주세요." });
    }
    if (!["여성", "남성"].includes(input.gender) || !["solar", "lunar"].includes(input.calendarType)) {
      return response.status(400).json({ message: "입력 형식을 확인해 주세요." });
    }

    const born = new Date(`${input.birthDate}T00:00:00+09:00`);
    if (!Number.isFinite(born.getTime()) || born > new Date()) {
      return response.status(400).json({ message: "올바른 생년월일을 입력해 주세요." });
    }
    ...
    if (record && record.reset > now && record.count >= 5) {
      return response.status(429).json({ message: "무료 분석을 여러 번 요청하셨어요. 한 시간 뒤 다시 이용해 주세요." });
    }
    attempts.set(ip, {
    ...
    if (!process.env.OPENAI_API_KEY) return response.status(503).json({ message: "무료 분석 기능을 준비하고 있어요." });
